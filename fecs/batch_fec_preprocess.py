...
def parse_metadata (fec_name):
    ...
    # Parsing if stretch or relax
    curve_type = fec_id[underscore_indexes[2]+4]

    # Parsing the curve number from the csv's.
    curve_number = fec_id[underscore_indexes[2]+5:len(fec_id)+1]
...
folder = "260906_BiotBact1_1016-601-Bact2_269NBsaILig_STdimerTArich_90mMKCl_2500uMMgCl2_15ng_ul_TYRNA_1umBeads"
...
opt_dict = {"bead1_diameter" : 0.81, # bead size in um
            "bead2_diameter" : 0.81, # bead size in um
            "correct_force_offset" : False, # keep false
            "apply_dist_offset" : True, # keep false unless there is brightfield camera data
            "video_trap_regress_cal" : False, # keep false (lumicks distance calibrations)
            "filter_type" : "boxcar"}
...
# Get only fecs
fecs = [x for x in files if "fec00" not in x]

#get only baselines
bsls = [x for x in files if "fec00" in x]
...
# NOTE: IF video_trap_regress_cal IS SET TO ON, THEN A LINEAR REGRESSION WILL BE
# MADE BETWEEN TRAP POSITION AND BEAD-TO-BEAD SEPARATION RECORDED BY THE CCD CAMERA
# USING THE BASELINE FILE. THE TRAP POSITION IN THE FEC WILL BE THEN CONVERTED
# TO BEAD-TO-BEAD SEPARATION BY MULTIPLYING IT BY SLOPE FOUND BY THE REGRESSION
# AND ADDING THE INTERCEPT. THIS ESSENTIALLY ASSUMES THAT A MOVEMENT OF X um
# IN TRAP POSITION IS EQUAL TO m*X um REAL MOVEMENT OF ONE OF THE BEADS.
# I FOUND THE SLOPE IS USUALLY 0.9 OR LOWER. IN OTHER WORDS, A MOVEMENT OF 1 um
# IN THE TRAP POSITION ONLY INCREASES/DECREASES THE BEAD SEPARATION BY 0.9 um or 
# LESS. I FOUND THAT IF YOU ASSUME THIS IS CORRECT, THE SIZE OF A CONTROL
# DNA HAIRPIN ESTIMATED BY MEASURING THE DISTANCE BETWEEN THE RIP AND A POINT
# OF EQUAL FORCE IN THE UNFOLDED ARM OF THE FEC WILL BE INCORRECTLY ESTIMATED.
# THIS SUGGEST DOING THIS CALIBRATION, AT LEAST FOR SHORT TETHERS LIKE 2 KB
# ONES IS NOT VALID. HENCE, MY SCRIPTS DO ALL CALCULATIONS WITH RAW TRAP POSITION
# EVEN IF video_trap_regress_cal IS SET TO ON. WHEN SET TO ON, THE "CORRECTED"
# TRAP POSITION, ALTHOUGH NOT USED FOR ANYTHING, WILL BE STORED IN A COLUMN
# IN THE DATAFRAMES CALLED cal_trappos1x. IF SET TO OFF (HIGHLY RECOMMENDED)
# THEN THIS COLUMN WILL BE PRESENT BUT EMPTY.
...
for i in fecs:
    ...
    fecs_dict[fec_basename] = preprocessed_fec
...
for i in fecs_dict.keys():
    fec_obj = fecs_dict[i]
    errcod = fec_obj.error_code
    if errcod == 0:
        d_3125 = fec_obj.fec_molextds
        f_3125 = fec_obj.fec_diffFds
        d_125 = pre.CK_filtfilt(d_3125, 25,3125)
        f_125 = pre.CK_filtfilt(f_3125, 25,3125)
...
# Create df for discarded based on the lists of failures
...
for i in fecs_dict.keys():
    ...
    if errcod != 0:
        tmplist = [0 for x in range(len(df_discarded.columns)-2)] + [errcod] + [i]
